...
function calculate_SN(agent::Person, model::ABM)::Float64
    ...
    if Graphs.nv(getfield(model, :space).graph) > 0 && !isempty(nearby_ids(agent, model)) # Check > 0 for any nodes
        for neighbor_id in nearby_ids(agent, model)
            neighbor = model[neighbor_id] # Access agent object using its ID
...
function calculate_PBC(agent::Person, model::ABM)::Float64
    ...
    # Corrected graph access and condition
    if Graphs.nv(getfield(model, :space).graph) > 0 && !isempty(nearby_ids(agent, model))
        for neighbor_id in nearby_ids(agent, model)
    ...
    for i in 1:num_actual_agents
        for j in (i+1):num_actual_agents
            ...
            if rand() < connection_prob
                # Standard and correct way to add an edge to the graph within GraphSpace
                Graphs.add_edge!(getfield(model, :space).graph, agent1.pos, agent2.pos)
            end
        end
    end
    # Standard and correct way to get number of edges
    println("Network formation complete. Number of edges: ", Graphs.ne(getfield(model, :space).graph))
end
...
function initialize_model(;
    ...
    model = ABM(
        ...
        properties = properties, scheduler = Agents.Schedulers.Randomly
    )
    ...
    for i in 1:num_agents
       ...
       employment_status_idx = rand(Distributions.Categorical(employment_probs_init ./ sum(employment_probs_init))) # Ensure probabilities sum to 1
       ...
       age_of_interest_val_val = generate_age_of_interest(current_skill_level_val)
...
# --- Agent Step Function ---
# (Keep agent_step! as it was, it's quite long to re-paste if unchanged)
...
# --- Model Step Function ---
# (Keep model_step! as it was)
function model_step!(model::ABM)
    model.current_time_months += model.delta_t_months
    # println("\n--- Model Step: Month $(model.current_time_months) ---") # Optional: uncomment for verbose step logging
    ...
    # Optional: Log aggregate data for debugging if not relying on run! mdata
    # num_migrated_total = count(a -> a.migration_stage == :migrated, allagents(model))
    # println("Month $(model.current_time_months): Migrated Agents = $num_migrated_total")
end

# --- Running the Simulation ---
function run_simulation()
    println("Starting Turkey-Germany Migration ABM...")
    model = initialize_model(num_agents = 100)
    ...
    agent_df, model_df = run!(model, agent_step!, model_step!, num_steps;
                              ...
                              when = 1:num_steps)
    println("\nSimulation Finished.")
    # Optional: Print more of the data or save it
    # println("Collected Agent Data (sample):"); display(first(agent_df, 10))
    # println("\nCollected Model Data:"); display(model_df)
    # using CSV
